...
arvoreB* split(arvoreB *x, int *infoMeio){
    ...
    y->numChaves = x->numChaves - metade - 1;
    x->numChaves = metade;
...
/*void contaAuxMax(arvoreB *r, int *k){//rascunho
    int i;

    for(i = 0; i < r->numChaves; i++){
        if(r->numChaves == (ORDEM - 1)) (*k)++;
    }
}

int contaNosMaxChaves(arvoreB *r){
    int count = 0;
    if(r != NULL){
        contaAuxMax(r, &count);
    }
...
int maxChave(arvoreB *r){
    if(r == NULL) return -1;
    else{
        int aux = 0;
         for(int j = 0; j < r->numChaves; j++){
            if(!isFolha(r)){
                aux = maxChave(r->filho[j + 1]);
            }
            else aux = r->chave[j];
        }
        return aux;
